...
| 티켓 | 내용 | 선행 | 상태 |
|---|---|---|---|
| UL-001 | 가져오기: NS-002·NS-003~005를 이 브랜치에 적용, 노드 목록 콘솔 지우기, 치트의 전투 중 거부 | — | 구현, 플레이 확인 대기 |
| UL-002 | 화면 루프: 업그레이드 ↔ 전투, `Start battle` 버튼 | UL-001 | 구현, 플레이 확인 대기 |
| UL-003 | 판이 업그레이드 표를 받는 자리, 전투 시작·종료 콘솔 표시, 계약 | UL-001 | 구현(계약 60개 통과), 플레이 확인 대기 |
| UL-004 | 문서(SYSTEM_CATALOG F02·F07 상태), 확인 | UL-002·003 | 문서 완료, 플레이 확인 대기 |
