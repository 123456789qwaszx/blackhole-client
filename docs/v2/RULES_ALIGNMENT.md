...
기준: 처음 점검은 dev `85e34bc`(2026-09-24, 코드 열람). M4 완료 후 "단계" 열에 반영 상태를 적었다.
...
| Core/Hq/Hq.cs | 위치만, 성장 제외 주석 | EXP 누적 후 Level 계산. 진행 효과는 별도 책임 | EXP 누적은 M4 완료, Level·진행 효과는 M5 |
| Core/Enemies/Enemy.cs | HP를 0까지 줄이지만 Death 상태 없음 | 즉시 사망 확정, 재피해/이동/선택 차단, 보상 1회 | M4 완료 |
| Core/World/World.cs | 이동 → 모든 Skill → 주기 출현 | 사망 처리 안전 지점, 생성 공급 변경, 후속 연쇄 큐 | 사망 처리는 M4 완료(복사한 대상 목록을 돌아 안전). 공급은 M5, 연쇄 큐는 M7 |
