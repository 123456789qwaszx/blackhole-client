...
1. **v1 보존**
   - 현재 커밋에 태그 `reference-v1`을 단다(로컬). 원격에 올릴지는 사용자가 정한다.
...
- 씬 정리 방법: (a) 사용자가 에디터에서 누락 스크립트를 지우고 빈 호스트를 붙인다, (b) 씬 YAML에서 v1 컴포넌트 블록을 지운다. 권장은 (a)다. YAML 직접 편집은 참조가 꼬이기 쉽다.
- `reference-v1` 태그를 원격에 올릴지.
