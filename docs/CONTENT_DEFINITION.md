...
| 관통 레이저 | 없음 | 실행 과정 자체를 보여 줘야 하므로 `TickCount`로는 부족하다. 예고 시작, 시작점, 확정 방향, 발사, 실제 경로를 Presentation이 정확히 소비할 수 있는 **기록 또는 읽기 전용 실행 상태** [제안]. CA-003에서 예고 중인 발사를 읽기 전용 `PendingShots`로 공개했다. 예고 표현에는 쓸 수 있다. 발사한 순간 목록에서 빠지므로, 발사 선·발사음을 한 번씩 내려면 사망 기록 같은 발사 기록이나 "이번 진행에서 발사됨" 정보가 더 필요할 수 있다(CA-005) |
