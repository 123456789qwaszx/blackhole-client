...
상위 계획: [CONTENT_AUTHORING_PLAN](CONTENT_AUTHORING_PLAN.md) — 이 문서는 **CA-001 산출물**이며 CA-005의 초안을 함께 담는다.
...
- R은 레이저가 처음 쓰는 공간 값이며, 노드가 바꾸는 Skill 수치가 아니다. 어디에 둘지는 CA-003에서 정한다 [미정].
...
현재 Core에는 무작위가 없다(출현 각도도 고정 간격이다). 레이저 시작점에는 무작위가 필요하다. 기준 상황을 반복 재현하려면(S12) Battle마다 seed를 가진 난수를 두고, seed를 조립 입력으로 받는다 [제안]. 계약 테스트는 seed를 고정한다.
...
| laser-unlock | 관통 레이저 | breaker-damage | 30 | 해금 | Skill `laser` | 전투 조립 | 코드 |
| growth-supply | 성장 공급 강화 | breaker-radius | 25 | Milestone 공급 수 +2 | Enemy `sample-light` | 공급 요청 | 있음 |
| laser-width | 레이저 굵기 | laser-unlock | 20 | 폭 +0.2 | Skill `laser` | 전투 조립 | 코드 |
...
## 5. Presentation / Audio 이벤트 — CA-005 초안
...
| 이벤트 | Core가 주는 것 | 현재 |
|---|---|---|
| Attack — Breaker 실행 | 실행 횟수 (`PassiveSkill.TickCount`). 화면은 이 값의 변화로 실행 연출을 낸다 | 있음. 맞힌 적의 수는 없다 |
| Attack — 레이저 예고·발사 | — | 없음. 예고 시작, 시작점, 확정 방향, 발사, 실제 경로가 필요하다 |
| Hit | HP 감소 | 간접 (화면이 HP 변화를 비교) |
| Death | 사망 기록 (번호·위치·종류·크기) | 있음 |
| Absorb | 사망 기록에서 시작하는 표현 전용 사건 | 있음 |
| Death Effect | 번개 적중 기록 (번호·출발·도착·대상) | 있음. 같은 번개의 적중끼리 묶는 값은 없음 |
| Growth — Level | HQ Level | 있음 (값 비교) |
| Growth — Milestone·시간 연장 | — | 없음. 일반 Level과 구별할 기록이 없다 |
| Cleanup 제거 | 목록에서 사라짐 | 있음. 사망 연출을 만들지 않는다 |

### 5.2 이벤트별 화면과 소리

리소스 이름과 소리 배정은 전부 [제안]이다.

| 이벤트 | 화면 | 소리 | 재생 단위 |
|---|---|---|---|
| Breaker 실행 | 범위 원 안쪽이 밝아짐 (있음) | 적중이 있으면 `breaker-hit` | 실행 1회 |
| 레이저 예고 | 얇은 선 | `laser-charge` | 발사 1회 |
| 레이저 발사 | 폭만큼 굵어졌다가 사라짐 | `laser-fire` | 발사 1회 |
| Hit | 짧은 번쩍임, 남은 HP 비율만큼 본래 색 (있음) | 없음. 공격 쪽 소리가 맡는다 | — |
| Death — Enemy A | 일반 파괴 | `break-small` | 사망 1개 |
| Death — Enemy B | 방전 파편 | `break-electric` | 사망 1개 |
| Absorb | HQ로 이동하며 작아짐 (있음) | 없음 | — |
| Death Effect | 적중마다 선 (있음) | `lightning-zap` | 번개 발동 1회 |
| Growth — Level | HQ가 커짐 (있음) | `hq-grow` | Level 상승 1회 |
| Growth — Milestone·시간 연장 | 남은 시간 강조 | `milestone` | Milestone 1개 |
| Cleanup | 즉시 제거 | 없음 | — |
...
CONTENT_AUTHORING_PLAN 7절은 재생 정책을 "실제 리소스를 연결하면서 결정"한다고 정했다. 아래 가설은 결정이 아니라, 리소스를 연결할 때 처음 시험할 값이다.

| 질문 (CONTENT_AUTHORING_PLAN 7절) | 가설 | 확인 방법 |
|---|---|---|
| Skill 발동 소리는 Tick당 1회인가? | 실행 1회에 한 번. 맞힌 수는 음량·음색 단계로 나타낸다. 아무도 맞히지 않은 Breaker 실행은 소리가 없다 | 0.5초 주기에서 빈 실행 소리가 거슬리는지 들어 본다 |
| 피격 소리는 대상마다 필요한가? | 필요 없다. 피격은 화면의 번쩍임, 소리는 공격 쪽 | 20개 동시 적중 장면 |
| 동일 프레임의 Death 소리는 몇 개까지인가? | 같은 소리는 한 프레임에 상한 N개까지 (N 미정) | 동시 사망 기준 상황 (S12) |
| Chain Lightning은 적중마다인가, 실행당인가? | 번개 발동 1회에 한 번 | 최대 연쇄 장면 |
| 동시 재생 상한을 넘으면 무엇을 생략하는가? | 새 재생을 버린다 | 상한을 낮춰 두고 들리는 차이 비교 |
| 새 Battle에서 남은 소리는? | 이전 Battle의 소리를 멈춘다 (S09의 잔여 표현 정리) | 재시작 반복 |
| 일시정지 중의 소리는? | 미정. 현재 연출은 실제 시간으로 흐른다 (임시) | S09 결정 |

가설대로면 한 프레임의 재생 수는 "소리 종류 수 × 상한"을 넘지 않는다. 동시 사망이 많아도 소리 수가 규칙으로 막힌다.

### 5.4 Core에 요구하는 것

Skill마다 필요한 것이 다르다. 모든 Skill을 위한 범용 공격 기록부터 만들지 않는다.

| 대상 | 지금 관찰할 수 있는 것 | 더 필요한 것 |
|---|---|---|
| Breaker | 실행 발생 (`TickCount`의 변화) | 없음. 5.3 가설의 "맞힌 적이 있을 때만 소리"를 쓰면 적중 유무가 필요하다. 지금은 같은 프레임의 HP 변화로 근사할 수 있지만 번개 피해와 섞인다. 정확한 값이 필요한지는 CA-005에서 소리를 붙여 보고 정한다 |
| 관통 레이저 | 없음 | 실행 과정 자체를 보여 줘야 하므로 `TickCount`로는 부족하다. 예고 시작, 시작점, 확정 방향, 발사, 실제 경로를 Presentation이 정확히 소비할 수 있는 **기록 또는 읽기 전용 실행 상태** [제안]. 형태는 CA-003에서 레이저에 필요한 만큼만 정한다 |
| 연쇄 번개 | 적중마다의 기록 | 5.3 가설(발동 1회에 소리 한 번)을 채택하면, 같은 번개의 적중을 묶는 발동 번호 [제안] |
| Growth | HQ Level | 일반 Level과 구별되는 Milestone 도달. 성장·시간 연장을 구별해 보여 줘야 한다 (S09 인수 기준) |
| Hit | HP 변화 | 없음. 피격 번쩍임은 지금처럼 HP 변화 비교로 충분하다 |
...
| 연결 키 | 표현 데이터 |
|---|---|
| Enemy ID | 외형, 색, 파괴 효과, 파괴음 |
| Skill ID | 범위·선 표시, 실행 효과, 실행음 |
| 사망 효과 종류 | 효과 연출, 효과음 |
| 전투 이벤트 | Level·Milestone·시간 연장의 연출과 소리 |

현재는 색만 코드(`SamplePresentation`)에 Enemy ID 문자열로 있다. ID가 틀려도 흰색으로 그려질 뿐 오류가 나지 않는다.

## 6. CA-002 전의 예상

위 명세를 현재 ContentData와 코드에 넣는다고 가정하고 적은 예상이다. **실제로 입력해 본 결과가 아니다.**
...
| 전투 영역 경계 R의 자리와 값, 화면 크기와의 관계 | CA-003, S02·S09 | 2.3 |
| 경로를 전투 영역 경계에서 끝낼지 | 원작 확인 후 CA-003 | 2.3 |
| 레이저 판정: 중심 기준 또는 Enemy 반지름 + 반폭 | CA-003 | 2.3 |
...
| 티켓 | 이 문서에서 넘기는 것 |
|---|---|
| CA-002 | 1절의 대표 콘텐츠를 현재 ContentData로 입력한다. 코드 구조는 고치지 않는다. 6.1의 표와 6.2의 불편을 실제 기록으로 바꾼다 |
| CA-003 | 2.4의 "코드" 차이. Aim Point Snapshot과 예고 상태, 선분 판정, 전투 영역 경계, seed 난수, 레이저 표현용 상태·기록 |
| CA-004 | 3.3의 해금 규칙, 3.4의 NodeId 요청, P1의 방향 |
| CA-005 | 5절 전체. 5.4의 기록 제안 |
