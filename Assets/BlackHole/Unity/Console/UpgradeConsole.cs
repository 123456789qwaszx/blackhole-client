...
namespace BlackHole.Unity
{
    ...
    internal sealed class UpgradeConsole : IDisposable
    {
        ...
        public UpgradeConsole(Transform parent, PlayerState player, NodeTree tree)
        {
            ...
            {
                ButtonOf(earn, "Earn" + label, "+" + label, GoldButtonWidth, () => _player.EarnGold(amount));
                ButtonOf(take, "Take" + label, "-" + label, GoldButtonWidth, () => ProgressCheats.TakeGold(_player, amount));
            }

            RectTransform nodes = Child(panel, "Nodes");
            HorizontalLayout(nodes, 8);
            ButtonOf(nodes, "UnlockAll", "Unlock all", NodeButtonWidth, () => ProgressCheats.UnlockAllNodes(_player, _tree));
            ButtonOf(nodes, "LockAll", "Lock all", NodeButtonWidth, () => ProgressCheats.LockAllNodes(_player));
