...
{
    // 화면 객체만 가진다. 매 프레임 World를 읽어 맞추고, 게임 상태를 바꾸지 않는다.
    ...
    internal sealed class WorldView : IDisposable
    {
        ...
        {
            Vector3 hq = ToScene(world.Hq.Position);
            _camera.transform.position = new Vector3(hq.x, hq.y, _camera.transform.position.z);
            _hq.transform.position = hq;
            _hqGlow.transform.position = hq;
        ...
        // 이전 판의 화면 객체를 정리한다. 지금은 판마다 생기는 객체가 없다(M2부터 Enemy View).
        public void Reset()
        {
        }
