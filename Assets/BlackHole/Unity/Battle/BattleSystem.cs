...
namespace BlackHole.Unity
{
    ...
    //      (받는 것은 산 노드에서 계산한 종류별 판 구성: 질량 단계·황금 비율·황금 배율).
    ...
    internal sealed class BattleSystem
    {
        ...
        // compositions는 종류별 판 구성(산 노드에서 계산한 값)이다. 조립이 판의 적 수치 표로 옮겨 적으므로, 뒤에 바뀌어도 이 판은 그대로다.
            ...
            IReadOnlyDictionary<EnemyDefinition, EnemyComposition> compositions)
        {
            ...
            // 1. 업그레이드에서 바뀐 수치 받기: 조립이 종류별 판 구성(산 노드에서 계산)으로 이 판의 적 수치 표를 확정한다.
            Session = SessionAssembler.CreateBattle(_content, players, stage, seed, compositions);
