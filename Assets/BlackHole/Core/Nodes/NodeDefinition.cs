...
{
    // 노드 하나: ID, 가격, 시작 노드인가, 사면 받는 업그레이드. 노드는 한 번만 산다.
    // 선(이웃)은 트리가 가진다(NodeTree). 업그레이드의 뜻은 모른다 — 산 노드의 업그레이드를 모아 UpgradeTable에 넘길 뿐이다.
    public sealed class NodeDefinition
    {
        ...
        internal NodeDefinition(string id, long price, bool isStart, IReadOnlyList<Upgrade> upgrades)
        {
